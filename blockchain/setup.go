...
func (setup *FabricSetup) Initialize() error {
	...
	chMgmtClient, err := setup.sdk.NewClient(fabsdk.WithUser(setup.OrgAdmin), fabsdk.WithOrg(setup.OrgName)).ResourceMgmt()
	if err != nil {
	...
	// Create channel
	req := resmgmt.SaveChannelRequest{ChannelID: setup.ChannelID, ChannelConfig: setup.ChannelConfig + "chainhero.channel.tx", SigningIdentity: orgAdminUser}
...
func (setup *FabricSetup) InstallAndInstantiateCC() error {
	...
	installCCReq := resmgmt.InstallCCRequest{Name: setup.ChainCodeID, Path: setup.ChaincodePath, Version: "v1", Package: ccPkg}
	...
	err = setup.admin.InstantiateCC(setup.ChannelID, resmgmt.InstantiateCCRequest{Name: setup.ChainCodeID, Path: setup.ChaincodePath, Version: "v1", Args: [][]byte{[]byte("init")}, Policy: ccPolicy})
	if err != nil {
